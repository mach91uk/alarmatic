...
import uk.mach91.autoalarm.dialogs.AddLabelDialog;
import uk.mach91.autoalarm.dialogs.AddLabelDialogController;
import uk.mach91.autoalarm.dialogs.TimePickerDialogController;
import uk.mach91.autoalarm.list.BaseViewHolder;
...
//                        mAlarmController.cancelAlarm(oldAlarm, false);
    ...
    protected void bindLabel(boolean visible, String label) {
        ...
        //if (label.toLowerCase().equals(AlarmPreferences.cancelAlarmHolidayLabel(getContext()).toLowerCase())) {
        //    if (DurationUtils.isOnHoliday(getContext(), getAlarm().ringsAt())) {
        //        newLabel += " (skipped as on holiday)";
        //    }
        //}
...
//    }
    ...
    void toggle(boolean checked) {
        ...
        if (mSwitch.isPressed()) { // filters out automatic calls from VH binding
            ...
            if (alarm.isEnabled()) {
                ...
                persistUpdatedAlarm(null, alarm, true);
            } else {
     ...
     */
    final void persistUpdatedAlarm(Alarm oldAlarm, Alarm newAlarm, boolean showSnackbar) {
        if (oldAlarm != null) {
            mAlarmController.cancelAlarm(oldAlarm, false, false);
        }
