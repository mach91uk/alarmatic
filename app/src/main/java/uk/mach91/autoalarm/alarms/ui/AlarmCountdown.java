...
public class AlarmCountdown extends TextView {
    ...
    private boolean mAlarmSkipHoliday = false;
    private boolean mCheckHoliday = false;
...
//        String text = DateUtils.formatElapsedTime(mRecycle, seconds);
        ...
        if (mCheckHoliday) {
            //if (mAlarmLabel.toLowerCase().equals(AlarmPreferences.cancelAlarmHolidayLabel(getContext()).toLowerCase())) {
            if (mAlarmSkipHoliday) {
                if (DurationUtils.isOnHoliday(getContext(), mBase)) {
                    text += " " + getContext().getString(R.string.alarm_skipping_on_holiday);
                }
            }
        }
        setText(text);
